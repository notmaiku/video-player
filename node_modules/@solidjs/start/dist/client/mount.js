...
export function mount(fn, el) {
    if (import.meta.env.START_ISLANDS) {
        ...
        const hydrateIslands = () => {
            ...
            islands.forEach((el) => el.dataset.id && assets.add(el.dataset.id));
            Promise.all([...assets].map(asset => import(/* @vite-ignore */ import.meta.env.MANIFEST["client"].chunks[asset.split("#")[0]].output.path)))
                .then(() => {
                islands.forEach((el) => {
